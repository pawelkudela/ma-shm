...
\definecolor{MYADDCOLOR}{rgb}{0.0, 0.0, 0.9} %blue color for added text
...
Due to the high computational demand of guided wave propagation \DIFdel{problems} \DIFadd{problems,} more and more researchers have been looking for possible ways of computation \DIFdel{speedup} \DIFadd{speed-up} over \DIFadd{the} last decade. One \DIFdel{of} possible solution is seen in using parallelisation via the Compute Unified Device Architecture (CUDA), which enables parallel computing on powerful graphics cards. A parallel algorithm of the Local Interaction Simulation Approach (LISA) for Lamb wave propagation modelling in \DIFdel{aluminum} \DIFadd{aluminium} plates exposed to temperature changes was developed by~Kijanka et~al.~\cite{Kijanka2013}. A~similar approach to model the nonlinear dynamic interactions between ultrasonic guided waves and fatigue cracks was utilised by Shen and Cesnik~\cite{Shen2017}.  
...
The developed spectral finite element for flat shell modelling is based on the Mindlin--Reissner \DIFdel{first order} \DIFadd{first-order} shear deformation theory. It has 36 nodes and 5 degrees of freedom at each node: displacement components \(u_0\) and \(v_0\) in neutral plane along \(x\) and \(y\) axis, respectively, transverse displacement \(w_0\) and two independent rotations of cross-section \(\varphi_x(x,y)\) and \(\varphi_y(x,y)\). The spectral shell element is schematically shown in figure~\ref{fig:spectral_shell_element}. The 36 node element was selected to be compliant with the mesh \DIFdel{coloring} \DIFadd{colouring} algorithm developed in~\cite{Kudela2016} in which the number of nodes in the element divided by 12 must result in a natural number. This implies that the computational load can be divided into 12 equal independent tasks. Moreover, \DIFadd{a} polynomial of 5\textsuperscript{th} order used for shape function construction is high enough to prevent locking on the thinner shells.
...
It should be noted that the displacement field resembles \DIFadd{a} combination of elementary two-mode membrane theory with three-mode Mindlin-Reisner theory resulting in \DIFadd{the} five-mode theory of shells. Such theory is suitable for \DIFadd{the} approximation of fundamental symmetric Lamb and shear wave modes in the frequency-thickness product range up to about 1~MHz\(\cdot\)mm and fundamental antisymmetric Lamb and shear wave modes in the frequency-thickness product range up to about 0.5~MHz\(\cdot\)mm (see~\cite{Ostachowicz2012} for more details).
...
\end{equation}
The quadrature \DIFdel{weight} \DIFadd{weights} \(w_i  0\), which are independent \DIFdel{form} \DIFadd{of} the element, are defined by:
\begin{equation}
...
\end{equation}
The components of \DIFadd{the} matrix \(\bm{\overline Q}_k^e\) are calculated for lamina rotated according to \DIFadd{the} stacking sequence. The transformation matrix of stiffness tensor \(\bm{Q}_k^e\) along with explicit formulas can be found in the literature~\cite{Vinson1987}.
It should be noted that in general both mass density \DIFadd{\(\rho_k\),} as well as stiffness tensor components \DIFadd{\(\bm{Q}_k^e\),} can have arbitrary values at GLL points but for \DIFdel{simplicity} \DIFadd{simplicity,} it is assumed that they are constant within \(k\)-th layer. Integral over \DIFadd{the} thickness of layers in (\ref{eq:dealm_plate_stiffness}) is evaluated analytically which leads to the following constants:
\begin{equation}
\eqalign{
...
where \(\bm{M}\) is the global mass (inertia) matrix, \(\bm{K}\) is the global stiffness matrix,  \(\bm{C}\) is the global damping matrix, \(\vec{U}\) is the vector of global degrees of freedom and~\(\vec{F}\) is the vector of the \DIFdel{time dependent} \DIFadd{time-dependent} excitation (in this particular case the vector of equivalent piezoelectric forces). The most efficient way to solve the (\ref{eq:motion}) is by using \DIFadd{an} explicit integration scheme. Assuming the central difference method:
\begin{equation}
...
It should be underlined that due to the orthogonality of shape functions and application of the GLL integration rule the mass matrix is diagonal. It has been shown that for the Lamb wave attenuation modelling it is possible to assume that the damping matrix is proportional to \DIFadd{the} mass matrix~\cite{Wandowski2017}. In such case calculation of displacements at the time step \(t + \Delta t\) is straightforward and does not require costly matrix inversion. However, the term \(\vec{F}^i=\bm{K}\vec{u}_t\) related to internal forces at the time step \(t\) is still computationally intensive. Moreover, assembly of the stiffness matrix is troublesome because it requires a lot of memory and limits the size of wave propagation problems which can be simulated. In order to alleviate these deficiencies, a parallel code is proposed in which assembly is performed at the internal force vector level without \DIFadd{the} necessity of stiffness matrix assembly. The proposed approach is very similar to the parallel implementation given in~\cite{Kudela2016}. However, due to analytic integration over \DIFadd{the} thickness and more complex displacement field in comparison to 3D solid spectral elements, the process of calculation of the term \(\vec{F}^i=\bm{K}\vec{u}_t\) is different and it is divided into two steps. In the first step stress-like components are calculated at the time moment \(t\) (index \(t\) in displacement vectors is omitted for clarity):
...
\end{array}\right],}
\end{equation}
where \(n\) is the total number of elements in the spectral element mesh. Sparse matrices of shape function derivatives are formed in \DIFadd{a} similar way, i.e.:
\begin{equation}
\bm{N},_{\xi} = \left[
\begin{array}{cccc}  
...
\end{equation}
where \(^T\) is the matrix transpose and \(\vec{W}\) is the vector resulting from \DIFadd{the} multiplication of integration weights and Jacobian determinant at appropriate GLL points:
\begin{equation}
\vec{W} = \vec{w}_{\xi}\,.*\vec{w}_{\eta}\,.*\left(\det\vec{J}\right).
\end{equation}
Since the internal forces are calculated for disjoined spectral elements, the assembly of a global vector \(\vec{F}^i\) is necessary. It can be performed according to mesh \DIFdel{coloring} \DIFadd{colouring} algorithm proposed in~\cite{Kudela2016}. The algorithm uniformly divides the nodes of spectral elements within the whole mesh into 12 sets. The sets are of the same size so the computation can be perfectly balanced between workers or resources can be uniformly divided within one graphics card.
...
\end{equation}
in which terms \(\vec{M}_0\), \(\vec{M}_1\) and \(\vec{M}_2\) are stored as vectors and \(./\) is element--wise division. In \DIFdel{particular} \DIFadd{particular,} all components in (\ref{eq:internal_forces}) as well as in (\ref{eq:vectorized_motion}) are implemented in Matlab Parallel Computing Toolbox as \verb|gpuArray|. In this way, the implementation is simple whereas CUDA GPU computation is transparent to the user. 
...
\end{equation}
where \(e_{31}\) and \(e_{32}\) are piezoelectric coupling constants, \(g_{33}\) is the extensional voltage constant, \(h_{pzt}\) is the thickness of \DIFadd{the} piezoelectric sensor. The submatrices and subvectors in (\ref{eq:pzt_sensing}) denoted by \(pzt\) subscript or superscript \DIFdel{corresponds} \DIFadd{correspond} to nodes of appropriate spectral elements which form piezoelectric sensors. The mean value of \(\vec{s}^{pzt}\) at \DIFadd{a} particular integration time step gives voltage value. It should be noted that the nodes of an element do not have electric potential degrees of freedom. The voltage is calculated directly from displacements.
...
\end{figure}
In the flat \DIFdel{shell} \DIFadd{shell,} all spectral elements are located at the neutral plane. There are two layers of elements at the delamination region with zero gap between them. In other words, at the delamination are two shell elements on top of each other that have the same nodal positions but the nodes are not merged. On the boundary of the delamination, the nodes of all shell elements are then again merged.

The contact effect is neglected (penetration of nodes is allowed) \DIFdel{but} \DIFadd{because of high computational costs involved for node penetration testing and additional computations which must be performed at each time step.
But} such \DIFadd{a} model seems to be accurate enough for the SHM applications. It should be noted that the delamination can be placed at \DIFadd{an} arbitrary interface between composite layers. The contribution to stiffness and inertia matrices is calculated separately for regions I, II and III, respectively taking into account \DIFdel{through thickness} \DIFadd{through-thickness} integrals over appropriate composite layers (see (\ref{eq:delam_plate_mass_dens}) and (\ref{eq:stiffness_contribution})).

\subsection{Interpolation on a uniform grid}

With the advent of laser vibrometry, many algorithms for damage visualization have been developed in which full wavefield data is used. Such algorithms can be tested also on synthetic data for \DIFadd{a} straightforward comparison of their efficiency. However, the mesh of spectral elements is usually unstructured due to the analysed features such as delamination as well as piezoelectric transducers. Moreover, the nodes of spectral elements are non-uniformly distributed and correspond to GLL points. This makes it difficult to apply damage visualization algorithms for such numerically simulated wavefields, especially when Fourier transforms are required where the grid of spatial points must be uniform. For this reason, an interpolation technique for the spectral element wavefield on a uniform grid of points on the top or bottom surfaces of the shell has been developed. 
...
\subsection{Computation \DIFdel{speedup} \DIFadd{speed-up} analysis}

The computational performance of the code was tested on a composite plate of material parameters introduced in Section~\ref{sec:ogw}. The plate had one embedded delamination. The applied excitation was a sinusoidal tone-burst signal of carrier frequency 50~kHz modulated by Hann window (five periods were used). It was applied to the piezoelectric transducer located at the centre of the plate on the top surface. The size of the plate has been increased \DIFdel{gradually,} \DIFadd{gradually} so that meshes contained more and more nodes (30\(\times\)30 cm, 40\(\times\)40 cm, 50\(\times\)50 cm, 70\(\times\)70 cm, 90\(\times\)90 cm, 100\(\times\)100 cm). The total run times for the investigated meshes and the corresponding numbers of degrees of freedom (NDOF) are given in table~\ref{tab:run_time}. The computation \DIFdel{speedup} \DIFadd{speed-up} for various meshes was calculated as follows:

\begin{equation}
speedup = \frac{CPU_{time}}{GPU_{time}}
\label{eq:speedup}
\end{equation}
where \(CPU_{time}\) is the average time of computation of one integration time step on a single core CPU (computations were performed on a workstation equipped with 6 \DIFdel{quad core} \DIFadd{quad-core} Intel Xeon X5660 2.8~GHz) and \(GPU_{time}\) is the average time of computation of one integration time step on GPU (computations were performed on Nvidia Tesla K20X). It can be seen in \DIFdel{figure~\ref{fig:speedup}} \DIFadd{figure~\ref{fig:speed-up}} that parallel version of the code run on Tesla K20X GPU card about 5--12\(\times\) faster than on single CPU (depending on the size of the problem). The trend of computation \DIFdel{speedup} \DIFadd{speed-up} is non-linear, of type \(\sqrt{NDOF}\). 
...
\caption{Computation \DIFdel{speedup} \DIFadd{speed-up} according to (\ref{eq:speedup});  NDOF - number of degrees of freedom.}
	\label{fig:speed-up}
\end{figure}
...
\end{itemize}
The following differences should be noted between the \DIFdel{model--based} \DIFadd{model-based} and \DIFdel{model--assisted} \DIFadd{model-assisted} structural health monitoring.
In the \DIFdel{model--based} \DIFadd{model-based} approach, numerical signals must match very well experimental signals. It is because numerical signals are used in an inverse method in which a certain objective function includes terms such as:
...
\begin{figure} [h!]
	\centering
		\subfloat[Wavefield plate]{\includegraphics{figure15.png}
\label{fig:wavefield_plate}}
		\hspace{5mm}
		\subfloat[SHM plate]{{\includegraphics{figure16.png}}
\label{fig:shm_plate}}
\caption{\DIFadd{Arrangement of piezoelectric transducers and defects}}
	\label{fig:plate_arrangement}
\end{figure}

\subsection{Defects}
In both \DIFdel{experiments} \DIFadd{experiments,} defects were simulated by an aluminium disk with a diameter of 10 mm \DIFdel{coupled} \DIFadd{and thickness 3 mm bonded} to the structure by \DIFdel{tacky tape.} \DIFadd{a double-sided adhesive tape so that it can be removed easily and moved to other location.} The underlying idea of using this reversible damage is to provide measurement data at various places on the structure so that model-assisted probability of detection (MAPOD) techniques can be developed~\cite{Eckstein2012}. The coordinates of defects in respect to the \DIFdel{lower left} \DIFadd{lower-left} corner of the plate which has been selected for numerical model validation are given in table~\ref{tab:defect_coordinates}.

In the numerical model, two defect modelling techniques were investigated: delamination as described in Section~\ref{sec:delam_model} and added mass (an \DIFdel{aluminum} \DIFadd{aluminium} disk with a diameter of 10~mm and thickness 3~mm). The delamination was modelled between 4\textsuperscript{th} and 5\textsuperscript{th} layer of composite laminate counting from the top surface.
...
\caption{Close up of spectral element mesh in the \DIFdel{damage} \DIFadd{damaged} area (green) and piezoelectric transducers no. 3 and 4.}
	\label{fig:spec_mesh_zoom}
\end{figure}
\clearpage

\subsection{Wavefield plate data set }
A comparison of experimental and numerical full wavefield data for selected time instances is presented in figure~\ref{fig:wavefields_comparison}. The wavefield patterns in the form of transverse velocities (see (\ref{eq:first_derivative_scheme})) are shown only for a \DIFadd{part of a} quarter of the plate \DIFadd{(see figure~\ref{fig:wavefield_plate})} on the surface opposite to the piezoelectric actuator (bottom surface). It can be noticed that \DIFdel{for both modelling techniques, i.e. delamination (figure~\ref{fig:num_frame72}, figure~\ref{fig:num_frame108}) and added mass (figure~\ref{fig:num_frame72_added_mass}, figure~\ref{fig:num_frame108_added_mass})} the wavefield patterns \DIFdel{matches} \DIFadd{match} the experimental data very well. However, \DIFdel{as expected, the interaction of the wave front with added mass is slightly different than with delamination: the added mass interaction with the wavefront is more pronounced compared} \DIFadd{S0} to \DIFdel{the case of the delamination, which} \DIFadd{A0 mode conversion} is \DIFdel{especially} \DIFadd{more} visible \DIFadd{in the numerical wavefield} at \DIFadd{the} time \DIFdel{frame} \(t=112.5\, \mu\)s. The \DIFadd{direct A0 mode} wave reflection pattern is \DIFadd{almost} the same in \DIFdel{each case. It can be attributed to the fact that the added mass locally changes overall inertia} \DIFadd{experimental} and \DIFdel{stiffness of the shell whereas delamination influences only stiffness.} \DIFadd{numerical wavefield.}
...
Despite these \DIFdel{differences} \DIFadd{differences,} it seems that reversible damage in the form of added mass \DIFdel{coupled} \DIFadd{bonded} to the structure by \DIFdel{tacky} \DIFadd{removable double-sided adhesive} tape is a good way for collecting benchmark data set. Such benchmark data set is a good starting point for \DIFadd{the} development of algorithms based on full wavefield data for visualization of defect location even in the form of delamination.


\begin{figure} [h!]
	\centering
	\subfloat[Experiment (added mass): \\ \hspace*{14pt} \(t=112.5\, \mu\)s]{\DIFaddfig{\includegraphics{figure23a.png}}
\label{fig:exp_frame64}}
   \hspace{0.5 cm} 	
   \subfloat[Experiment (added mass): \\ \hspace*{14pt}  \(t=168.7\, \mu\)s]{\DIFaddfig{\includegraphics{figure23b.png}}
\label{fig:exp_frame100}}\\	
   \subfloat[Simulation (added mass): \\ \hspace*{14pt} \(t=112.5\, \mu\)s]{\DIFaddfig{\includegraphics{figure23e.png}}
\label{fig:num_frame72_added_mass}}
   \hspace{0.5 cm} 	
   \subfloat[Simulation (added mass): \\ \hspace*{14pt}  \(t=168.7\, \mu\)s]{\DIFaddfig{\includegraphics{figure23f.png}}
...
\begin{figure} [h!]
		\centering
		\subfloat[Simulation (delamination): \\ \hspace*{14pt} \(t=200\, \mu\)s]{\DIFaddfig{\includegraphics{figure24a.png}}
\label{fig:num_frame128_top}}
		\hspace{0.5 cm} 	
		\subfloat[Simulation (added mass): \\ \hspace*{14pt} \(t=200\, \mu\)s]{\DIFaddfig{\includegraphics{figure24b.png}}
...
The greatest difference between experimental and numerical wave packet amplitude is in figure~\ref{fig:path4_8} at about 570~\(\mu\)s. \DIFdel{It can be attributed to the damping coefficient selected in numerical simulation.} \DIFadd{ It can be attributed to factors such as bonding layer between piezoelectric transducer and the host structure which is not taken into account in the model, differences between assumed and actual piezoelectric properties and damping model selected in numerical simulations.} It should be underlined that the material damping is very important because it affects attenuation of guided wave modes and in turn particular wave packets in signals. It has been assumed in the model that the damping matrix is proportional to the mass matrix \(\bm{C} = \alpha \bm{M}\).
\DIFdel{ In the current modelling approach, the coefficient \(\alpha\) was arbitrary selected (\(\alpha = 4000\)).} \DIFadd{In the current modelling approach, the coefficient \(\alpha\) was estimated according to the procedure described in section~\ref{sec:damping} as \(\alpha = 3969\).}
...
\clearpage
The conducted signal analysis leads to the conclusion that a model-based approach in which numerical signals are directly compared to experimental one is not feasible with the current model. However, the developed model is perfectly suited for model-assisted approaches. It is because the overall wave behaviour such as wave amplitude change due to damage and interaction of elastic waves with damage is properly modelled. It especially visible in case of differential signals presented in~figure~\ref{fig:path1_7_diff}. The differential signals are calculated as 'baseline' minus 'added mass' signals. The operation is performed separately for experimental and numerical signals. It can be seen that both in experimental and numerical differential signals, wave packets at about\DIFdel{ 200~\(\mu\)s and 400~\(\mu\)s} \DIFadd{650~\(\mu\)s} gives the greatest difference due to added mass. Even the amplitude of differential signals is on the same level.

\DIFadd{For the other transducer pairs such as 3--10 (figure~\ref{fig:path3_10_diff}) and 4--8 (figure~\ref{fig:path4_8_diff}), the amplitude of differential signals is even greater because the added mass is directly on the path from actuator to sensor (compare figure~\ref{fig:shm_plate}).
}
\begin{figure} [h!]
	\centering
\DIFdelfig{\scalebox{1}{\includegraphics{figure11.png}}}
\DIFaddfig{\scalebox{1}{\includegraphics{figure20.png}}}
\caption{Comparison of experimental and numerical differential signals \DIFdel{('baseline' minus 'added mass')} for transducer pairs \DIFadd{1--7,} frequency 40~kHz.}
	\label{fig:path1_7_diff}
\end{figure}
\begin{figure} [h!]
	\centering
		\DIFaddfig{\scalebox{1}{\includegraphics{figure21.png}}}	
		\caption{\DIFadd{Comparison of experimental and numerical differential signals for transducer pairs 3--10, frequency 40~kHz.}}
		\label{fig:path3_10_diff}
	\end{figure}
\begin{figure} [h!]
	\centering
		\DIFaddfig{\scalebox{1}{\includegraphics{figure22.png}}}
		\caption{\DIFadd{Comparison of experimental and numerical differential signals for transducer pairs 4--8, frequency 40~kHz.}}
...
\subsection{\DIFadd{Damping effect} \label{sec:damping}}
\DIFaddcite{The most common viscous damping description is the Rayleigh damping model, in which a linear combination of mass and stiffness matrices is used. The  damping matrix can be defined as:
\begin{equation}
...
Without going into details the relation between damping coefficient \(\eta\) and proportionality damping coefficients \(\alpha\) and \(\beta\) given in~(\ref{eq:rayleigh_damping}) can be written as~\DIFaddcite{\cite{Gresil2015}}:
\begin{equation}
...
\clearpage
\section{Conclusions}
A novel parallel implementation of the time domain spectral element method has been developed. The method utilizes flat shell spectral elements based on \DIFdel{first order} \DIFadd{first-order} shear deformation theory as a basis of implementation. The developed code is highly parallel and enables GPU computation. It allows up to 12\(\times\) computation \DIFdel{speedup} \DIFadd{speed-up} in comparison to CPU. It also does not require assembly of \DIFadd{the} stiffness matrix. Therefore, larger guided wave propagation problems can be solved than in case of conventional finite element approach.
