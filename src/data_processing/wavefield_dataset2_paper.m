...
Cmap = gray(256); 
caxis_cut = 0.8;
fig_width =4; % figure widht in cm
fig_height=4; % figure height in cm
    ...
    for test_case=tasks
        ...
        for n=1:length(motion)
            ...
            for s=1:length(shell_surface)
                ...
                filename=[figure_output_path,num2str(frame_no(1)),'_flat_shell_',variable_name,'_',num2str(test_case),'_',num2str(Nx),'x',num2str(Ny),shell_surface{s}];
                if(overwrite||(~overwrite && ~exist([filename,'.png'],'file')))   
                    try
                        ...
                        [nx,ny,nft]=size(Data);
                        for k=frame_no
                            frame_filename=[figure_output_path,num2str(k),'_flat_shell_',variable_name,'_',num2str(test_case),'_',num2str(Nx),'x',num2str(Ny),shell_surface{s}];
                           ...
                           caxis([caxis_cut*Smin,caxis_cut*Smax]);
                            ...
                            print(frame_filename,'-dpng', '-r600'); 
                        end
                        end
                    catch
