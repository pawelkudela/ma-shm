...
model_input_path=['/pkudela_odroid_sensors/ma-shm/data/interim/num/flat_shell/','flat_shell_rand_',num2str(m),'_out'];
        %%
    for test_case=tasks
        ...
        for n=1:length(motion)
            ...
            for s=1:length(shell_surface)
                ...
                filename=[processed_output_name,modelname,shell_surface{s}];
                if(overwrite||(~overwrite && ~exist([filename,'.png'],'file')))   
                    try
                        ...
                        % S7(51,101)
                        % S8(51,400)
                        S(test_case,1,:)=Data(101,450,:);
                        S(test_case,2,:)=Data(400,450,:);
                        S(test_case,3,:)=Data(450,450,:);
                        S(test_case,4,:)=Data(450,450,:);
                        S(test_case,5,:)=Data(400,51,:);
                        S(test_case,6,:)=Data(101,51,:);
                        S(test_case,7,:)=Data(51,101,:);
                        S(test_case,8,:)=Data(51,400,:);
