function [nodes3D_new,coords3D_new]=connect_3Dsolid_top(NofElNodesx,NofElNodesy,NofElNodesz,solidEl,nodes3D,coords3D,solid_t)
...
cc=0;
 if(isempty(solidEl))
     ...
     for m=1:nsolid
         ne=solidEl(m);
         c=c+1;
         nodes3Dsolidlay=[nodes3Dsolidlay,nodes3D(ne,(NofElNodesz-1)*NofElNodesx*NofElNodesy+1:NofElNodesx*NofElNodesy*NofElNodesz)];
         nodes3Dsolid(c,1:NofElNodesx*NofElNodesy)=nodes3D(ne,(NofElNodesz-1)*NofElNodesx*NofElNodesy+1:NofElNodesx*NofElNodesy*NofElNodesz);
     end
...
for m=1:nsolid
    ...
    for k=1:length(ksi)
        coords3D_new(nodes3D_new(fen+c,(NofElNodesx*NofElNodesy)*(k-1)+1:k*NofElNodesx*NofElNodesy),3)=coords3D_new(nodes3D_new(ne,NofElNodesx*NofElNodesy*NofElNodesz-NofElNodesx*NofElNodesy+1:NofElNodesx*NofElNodesy*NofElNodesz),3)+(ksi(k)+1)/2*solid_t;
    end
