function [nodes3D_new,coords3D_new]=connect_3Dsolid_bottom(NofElNodesx,NofElNodesy,NofElNodesz,solidEl,nodes3D,coords3D,solid_t)
...
for m=1:nsolid
    ...
    for k=length(ksi):-1:1
        coords3D_new(nodes3D_new(fen+c,(NofElNodesx*NofElNodesy)*(k-1)+1:k*NofElNodesx*NofElNodesy),3)=coords3D_new(nodes3D_new(ne,1:NofElNodesx*NofElNodesy),3)-(ksi(k)+1)/2*solid_t;
    end
