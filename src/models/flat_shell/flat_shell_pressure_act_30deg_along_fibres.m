...
pressure_field_name = '/pkudela_odroid_laser/acoustic_excitation/Pa_1ACT_25mm_30stp_along';
% input for constant parameters
input_file_no = [56]; % 
tasks=[1];
mode='gpu'; % options: mode='cpu';mode='gpu';
meshfile=fullfile('mesh','rect_70x70');
...
for test_case=tasks
    ...
    if(overwrite||(~overwrite && ~exist([output_name,'time',num2str(test_case),'.mat'],'file')))
        ...
        try
            ...
            %% RUN MODEL
            t_frames=main_flat_shell_pressure_excit_c(pressure_field_name,test_case,input_no,meshfile,mode,output_name,tasks);
