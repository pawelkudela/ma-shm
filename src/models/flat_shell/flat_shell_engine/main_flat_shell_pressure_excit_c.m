function t_frames=main_flat_shell_pressure_excit_c(pressure_field_name,test_case,input_no,meshfile,mode,output_name,tasks)
% main_flat_shell_force_excit_c   Compute wavefield by using flat shell spectral elements and c tensor for material prop 
%                                 The case of force excitation
% Syntax: main_flat_shell_force_excit_c(test_case,meshfile,mode,output_name)
% 
% Inputs: 
%    pressure_field_name - filename of mat file containing pressure distribution on the plate (Data), 
%    on uniform grid defined by spacing dx, dy, dt
%    force_coords - coordinates at which nodal tranverse force is applied,
%    double, dimensions [1,2], defined in input file
%    output_coords - coordinates at which nodal outputs are stored, double,
%    dimensions [n,2], defined in input file
...
% Example: 
%    main_flat_shell_force_excit_c(test_case,meshfile,mode,output_name)
%    main_flat_shell_force_excit_c(1,'mesh1','gpu',output_name) 
% 
...
% load pressure
load(pressure_field_name); % Data, dx [m], dy [m], dt [1e3*s]
...
for fn=1:size(Data,3)
    v1=reshape(Data(:,:,fn),[size(Data,1)*size(Data,2),1]);
    x1=reshape(X,[size(Data,1)*size(Data,2),1]);
    y1=reshape(Y,[size(Data,1)*size(Data,2),1]);
    F = scatteredInterpolant(x1,y1,v1);
    % interpolated pressure
    p(:,fn) = F(coords(:,1),coords(:,2));
end
%
pztEl=[]; % overwrite what is in the mesh and exclude piezoelectric elements
pztnum=[];
...
if(isempty(delamEl))
    % host structure elastic constants for the reference case
     if(isempty(pztEl))
        [~, m11, I11, a11, a22, a12, a16, a26, a66, a44, a45, a55, b11,b12,b16,b22,b26,b66,d11, d12, d16, d22, d26, d66]=composite_plate_c_delam(h, h1, h2, rho, q11,q12,q22,q44,q55,q66,alpha,lay,delamEl,den_above,den_under,delamination_layer,fen,NofElNodes);
...
%     FIY=zeros(fen*NofElNodes,1,'double','gpuArray');
    ...
    Fp=zeros(5*fen*NofElNodes,1,'double','gpuArray');
...
for nn=1:nft
    tstart = tic;
    t_sem = (nn-1)*tt/(nft-1);
    pt=interp1(t1',p',t_sem,'linear',0);
...
% transform from global displacement vector to local elemental vector
   ...
   if(~isempty(pztEl))
    ...
    switch mode
        ...
        F_pressure=zeros(dof,1,'double','gpuArray');
        case 'cpu'
        F=zeros(dof,1);
        F_pressure=zeros(dof,1);
    end
 ...
 switch assembly
     case 'trad'
        for ne=1:fen
             ...
             F_pressure(5*nodes(ne,:),1)=F_pressure(5*nodes(ne,:),1)+fl(n1:n2);
        end 
        clear fu fv fw fl 
     case 'mesh'  
        F2(1:5:end)=fu;
        ...
        for ibasket=1:12
            ...
            F_pressure(I_G_dofs(:,ibasket))=F_pressure(I_G_dofs(:,ibasket))+Fp(I_L_dofs(:,ibasket)); 
        end
%         F(IG1)=F(IG1)+F2(IL1);
%         F(IG2)=F(IG2)+F2(IL2);
%         F(IG3)=F(IG3)+F2(IL3);
...
%         F(IG12)=F(IG12)+F2(IL12);
    ...
    if(isempty(pztEl))
        ...
        F=-F+mg2.*U+F_pressure-mg.*uold+a1*cg.*uold; % pressure excitation
    else
...
% ================ [main_flat_shell_pressure_excit_c.m] ================  
