function t_frames=main_flat_shell_single_prec(test_case,input_no,meshfile,mode,output_name,tasks)
...
switch mode
    case 'gpu'
        Npx=single(Npx); Npy = single(Npy); % single precision sparse arrays not supported!
        Npx=gpuArray(Npx);
        Npy=gpuArray(Npy);
...
switch mode
    case 'gpu'
        Iu=uint32(Iu);
        Iu=gpuArray(Iu);       
        U=zeros(dof,1,'single','gpuArray');
%     UX=zeros(fen*NofElNodes,1,'single','gpuArray');
%     UY=zeros(fen*NofElNodes,1,'single','gpuArray');
%     UZ=zeros(fen*NofElNodes,1,'single','gpuArray');
%     FIX=zeros(fen*NofElNodes,1,'single','gpuArray');
%     FIY=zeros(fen*NofElNodes,1,'single','gpuArray');
    F2=zeros(5*fen*NofElNodes,1,'single','gpuArray');
    mgL=zeros(5*fen*NofElNodes,1,'single','gpuArray');
    mg=zeros(dof,1,'single','gpuArray'); % global mass matrix
...
switch mode
    case 'gpu'
    dv=zeros(dof,1,'single','gpuArray'); % damping vector
    case 'cpu'
...
switch mode
    case 'gpu'
    uold=zeros(dof,1,'single','gpuArray');
%     unew=zeros(dof,1,'single','gpuArray'); % memory prealocation
%     v=zeros(dof,1,'single','gpuArray');
    case 'cpu'
    uold=zeros(dof,1);
...
switch mode
    ...
    ne=zeros(1,1,'single','gpuArray'); 
    NofElNodes=gpuArray(NofElNodes);
    n1=zeros(1,1,'single','gpuArray');
    n2=zeros(1,1,'single','gpuArray');
    case 'cpu'
...
if(isempty(pztEl))
    switch mode
        case 'gpu'
            Fi=zeros(dof,1,'single','gpuArray');  
        case 'cpu'
            Fi=zeros(dof,1);
    end   
else
