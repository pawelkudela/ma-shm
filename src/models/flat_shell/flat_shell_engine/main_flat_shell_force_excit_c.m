function t_frames=main_flat_shell_force_excit_c(test_case,input_no,meshfile,mode,output_name,tasks)
...
% compute output dofs
I=zeros(length(output_coords),1);
for k=1:length(output_coords)
    [~,I(k)] = min(sqrt( (coords(:,1) - output_coords(k,1)).^2+( coords(:,2) - output_coords(k,2)).^2) );
end
outputs=5*I; % transverse displacements
% compute force dofs
[~,J] = min(sqrt( (coords(:,1) - force_coords(1,1)).^2+( coords(:,2) - force_coords(1,2)).^2) );
force_dof=5*J;
...
if(isempty(pztEl))
    Fi(force_dof)=1;Fi=Fi*V0; % force excitation according to dof defined in output  
end
...
% ================ [main_flat_shell_force_excit_c.m] ================  
