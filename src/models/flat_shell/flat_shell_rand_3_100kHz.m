...
modelname = name; 
image_label_path = prepare_model_paths('raw','num',modelfolder,'automesh_delam_rand3'); % mesh parameters and labels
%image_label_path = prepare_model_paths('raw','num',modelfolder,'automesh_delam_rand_100kHz'); % mesh parameters and labels
...
for test_case=tasks
    ...
    if(overwrite||(~overwrite && ~exist([output_name,'time',num2str(test_case),'.mat'],'file')))
        ...
        try
            ...
            %% RUN MODEL
            t_frames=main_flat_shell_c_degradation(test_case,input_no,meshfile,mode,output_name,tasks);
            %
