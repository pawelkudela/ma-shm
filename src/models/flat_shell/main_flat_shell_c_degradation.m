function t_frames=main_flat_shell_c_degradation(test_case,input_no,meshfile,mode,output_name,tasks)
% MAIN_FLAT_SHELL_C   Compute wavefield by using flat shell spectral elements and C tensor for material prop
%                                It considers degradation degree of C tensor in delamination region
% 
% Syntax: main_flat_shell_c(test_case,meshfile,mode,output_name)
% 
% Inputs: 
...
% Example: 
%    main_flat_shell_c(test_case,meshfile,mode,output_name)
%    main_flat_shell_c(1,'mesh1','gpu',output_name) 
% 
...
if(isempty(delamEl))
    ...
    if(isempty(pztEl))
        ...
        if(damage_degree == 0)
            ...
            [~, m11, I11, a11, a22, a12, a16, a26, a66, a44, a45, a55, b11,b12,b16,b22,b26,b66,d11, d12, d16, d22, d26, d66]=composite_plate_c_delam_pzt_degradation(h, h1, h2, rho,C,damage_degree,alpha,lay,delamEl,den_above,den_under,delamination_layer,fen,NofElNodes,rho_pzt,pzt_thickness,Qpzt,pztEl);
        end
    end
end
...
dv(1:5:end)=etad_xy; % in-plane damping factor u
dv(3:5:end)=etad_xy; % in-plane damping factor v
dv(5:5:end)=etad_z; % out-of-plane damping factor w
dv(2:5:end)=etad_z; % out-of-plane damping factor fix
dv(4:5:end)=etad_z; % out-of-plane damping factor fiy
% damping at delamination
for j=1:length(den_above)
    ...
    for ne=DelamEl
        c=c+1;
        dv(5*nodes(ne,:)-4,1)=etad_delam_xy; % u
        dv(5*nodes(ne,:)-2,1)=etad_delam_xy; % v
        dv(5*nodes(ne,:),1)=etad_delam_z; % w
        dv(5*nodes(ne,:)-3,1)=etad_delam_z; % fix
        dv(5*nodes(ne,:)-1,1)=etad_delam_z; % fiy
    end
end
