function [spec_element_nodes,spec_coords,boundary_nodes] = quad2spec_boundary(q,quad_coords,N)
% QUAD2SPEC_BOUNDARY   convert quad nodes mesh into spectral element mesh 
%    only for linear quad elements 
%    collect also list of nodes at the boundary
% 
% Syntax: [spec_element_nodes,spec_coords,boundary_nodes] = quad2spec_boundary(q,quad_coords,N)
% 
% Inputs: 
...
% 
% Example: 
%    [spec_element_nodes,spec_coords,boundary_nodes] = quad2spec_boundary(q,quad_coords,N)
% 
...
ibn =0; % index of boundary nodes
boundary_nodes=zeros(nQuadElements*n,1);
m=[1,2,3,4,1];
   ...
   [X,Y]=spec_nodes(ksi,quad_coords(q(k,:),1),quad_coords(q(k,:),2));
     ...
     for j=1:4
         ...
         switch K(q(k,m(j)),q(k,m(j+1)))     
             case 1
                 ibn=ibn+1;
                 boundary_nodes(ibn)=q(k,m(j));
%                 for i=1:mol
%                      c=c+1;
%                      ck=ck+1;
%                      ibn=ibn+1;
%                      boundary_nodes(ibn)=ck;
%                      spec_element_nodes(cc,mo(c))=ck;
%                      spec_coords(ck,1)=X(mo(c));spec_coords(ck,2)=Y(mo(c));
%                 end
                %
                boundary_nodes(ibn+1:ibn+mol)=ck+1:ck+mol;
                spec_element_nodes(cc,mo(c+1:c+mol))=ck+1:ck+mol;
                spec_coords(ck+1:ck+mol,1)=X(mo(c+1:c+mol));spec_coords(ck+1:ck+mol,2)=Y(mo(c+1:c+mol));
                c=c+mol;
                ck=ck+mol;
                ibn=ibn+mol;
                %
                ibn=ibn+1;
                boundary_nodes(ibn)=q(k,m(j+1));
             case 2
...
%                  end
                ...
                c=c+mol;
                ck=ck+mol;
                Kp(q(k,m(j+1)),q(k,m(j))).q(1:mol)=ck+1:ck+mol;
             case 3
                 c1=mol;
                 for i=1:mol
                     c=c+1;
...
boundary_nodes =  boundary_nodes(1:ibn,1);
boundary_nodes = unique(boundary_nodes);
%---------------------- END OF CODE---------------------- 

% ================ [quad2spec_boundary.m] ================  
