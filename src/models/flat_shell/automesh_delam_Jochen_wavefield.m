...
a_set=[0.005]; % set of delamination semi-major axis
b_set=[0.005]; % set of delamination semi-minor axis
rotAngle_set = [0]; % set of delamination rotation angles [0:180)
r=0.005; % pzt radius
...
counter = 0;
% delamination position: only one location
...
for ib=1:length(b_set)
    ...
    for ia=1:length(a_set)
        ...
        if(a == b)
            counter = counter + 1;
            rotAngle = 0;
            mesh_filename = ['delam_Jochen_wavefield','_a_',num2str(a*1e3),'mm_b_',num2str(b*1e3),'mm_angle_',num2str(rotAngle)]; 
            mesh_parameters(counter).position_no = 1;
