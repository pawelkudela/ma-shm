...
modelname = name; 
image_label_path = prepare_model_paths('raw','num',modelfolder,'automesh_delam_Jochen_wavefield'); % mesh parameters and labels
...
NofMeshes = length(mesh_filename);
actuator_no = 1; % one pzt only in the centre
tasks=[1:NofMeshes];
...
%% input for figures
selected_frames = [64,100,120,128,256,512];
%ColorMapName = 'jet';
...
%%
for test_case=tasks
    meshfile = fullfile('mesh',mesh_filename{test_case});
